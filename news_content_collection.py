...
def get_web_archive_results(search_url):
    try:
        archive_url = "http://web.archive.org/cdx/search/cdx?url={}&output=json".format(search_url)
...
def get_website_url_from_archive(url):
    """Get the url from http://web.archive.org/ for the passed url if it exists"""
    archived_results = get_web_archive_results(url)
    if archived_results:
        modified_url = "https://web.archive.org/web/{}/{}".format(archived_results[0][1], archived_results[0][2])
        return modified_url
    else:
        return None
...
def crawl_news_article(url):
    news_article = crawl_link_article(url=url)

    # If the news article couldnt be fetched from the original website, fetch from archive if it exists.
...
def collect_news_articles(news_list, news_source, label, config: Config):
    ...
    for news in tqdm(news_list):
        ...
        if news_article:
            json.dump(news_article, open("{}/{}/news_content.json".format(save_dir, news.news_id),
                                         "w", encoding="UTF-8"))


class NewsContentCollector(DataCollector):
    def __init__(self, config):
        super(NewsContentCollector, self).__init__(config=config)
