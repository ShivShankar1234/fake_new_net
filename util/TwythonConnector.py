...
class TwythonConnector:
    def __init__(self, key_server_url, key_file):
        self.streams = []
        self.init_twython_objects(key_file)
        self.url = "http://" + key_server_url + "/get-keys?resource_type="
        self.max_fail_count = 3

    def init_twython_objects(self, keys_file):
        """
        CHANGE THIS TO USE CONNECTION MODE 0???
        Reads the keys file and initiates an array of twython objects
        :param keys_file:
        :return:
        """
        keys = json.load(open(keys_file, 'r'))
        # These keys come from you twitter account
        # To make key_file see: https://www.youtube.com/watch?v=56EAU6liJmA
        for key in keys:
    ...
    def get_twython_connection(self, resource_type):
        """
        Returns the twython object for making the requests.
        If all the Twitter Access Keys have reached the usage limits, the sleeps
        :param resource_type:
        :return:
        """
        ...
        while True:
            ...
            if response.status_code == 200:
                ...
                if response["status"] == 200:
                    print("resouce id: {}".format(response["id"]))
                    return response["id"]
                else:
...
# Sanity Testing (Comment out later)
if __name__ == "__main__":
    connector = TwythonConnector("127.0.0.1",
                                 "/Users/shivshankar/desktop/python_projects/fake_news_net/resources/keys_file.txt")
    for i in range(1000):
        connection = connector.get_twython_connection("get_follower_ids")
